...
def plot_video_metrics_event(metadata: pl.DataFrame, timeseries_df: pl.DataFrame, save_path: str = None, show: bool = True):
    ...
    subplot_titles_ = ['Duration (sec)', 'Mean Delta Videos', 'Std Delta Videos', 'Capitalization Ratio', 
                    'Live Videos %: <br> US', 'Live Videos %: <br> Europe ', 'Live Videos %: <br> Asia']
    num_subplots = 4*3+3
    row_num = 2
    col_num = 4 
    ...
    for i, title in enumerate(subplot_titles_):
        ...
        annotations.append(dict(
            ...
            y = 1 - (row - 1) / row_num + 0.02 if row == 1 else 1 - (row - 1) / row_num - 0.12,
            text=title,
            showarrow=False,
            xref="paper",
            yref="paper",
...
def plot_video_metrics_event_region(metadata: pl.DataFrame, timeseries_df: pl.DataFrame, save_path: str = None, show: bool = True):
    ...
    row_num = 2
    col_num = 4 
    subplot_titles_ = ['Duration (sec)', 'Mean Delta Videos', 'Std Delta Videos', 
                 'Capitalization Ratio', ' ', 
                 'Live Video %:<br>Geopolitical', 'Live Video %:<br>Environmental']
    ...
    for j, region in enumerate(geographical_location):
        ...
        for idx, column in enumerate(vid_feature_columns):
            ...
            if row == 2 and col == 1:
                ...
                # print("circle plot for asia")
                live_values_df=vid_features.filter(pl.col('event_type') == 'geopolitical')['is_live'].value_counts()
                live_values_df = live_values_df.to_pandas()
                live_values_df['is_live'] = live_values_df['is_live'].map({0: 'Not Live', 1: 'Live'})
                fig.add_trace(
                    go.Pie(labels=live_values_df['is_live'], 
